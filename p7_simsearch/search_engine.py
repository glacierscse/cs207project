...
class searchEngine:
	...
	def __init__(self):
		...
		for id in range(NUM_TS):
			...
			self.ts_list.append(pickle.load(open('../p7_simsearch/ts_data/ts_%d.dat'%id, 'rb')))
		
		vp_ids = pickle.load(open('../p7_simsearch/vantage_pts.dat', 'rb'))
		for id in vp_ids:
			self.vp_list.append((self.ts_list[id], id))


	def search_by_ts(input_ts, n):
		...
		for i in range(n):
			cur_dist = dist[i][0]
			cur_vt_id = dist[i][1]
			cur_db = connect('p7_simsearch/ts_db_index/ts_' + cur_vt_id + '.db')
			...
			for (ds, Id) in dist_ids:
				if Id not in id_set:
					id_set.add(Id)
					cur_ts = pickle.load(open('p7_simsearch/ts_data/ts_' + Id + '.dat', 'rb'))
	...
	def getts_with_id(input_id, sm):
		return sm.get(input_id)

	def search_by_id(input_id, n,sm):
